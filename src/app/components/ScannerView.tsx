...
export function ScannerView() {
  ...
  const addProductToCart = (product: Product) => {
    setCart((prev) => {
      ...
      if (existing) return prev.map((item) => item.type === 'product' && item.id === product.id ? { ...item, quantity: item.quantity + 1 } : item);
  ...
  const addSerieToCart = (serie: Serie) => {
    ...
    setCart((prev) => {
      ...
      return [...prev, { type: 'serie' as const, id: `serie-${serie.id}`, serieId: serie.id, name: `📦 ${serie.name} (${remaining} items)`, image: serie.image, price: serie.sellingPrice, quantity: 1 }];
    });
  };
  ...
  const handleScan = (e: React.FormEvent) => {
    ...
    setBarcodeInput(''); inputRef.current?.focus();
  };
  ...
  const handleCheckout = () => {
    ...
    cart.forEach((item) => {
      if (item.type === 'serie-item') sellSerieItem(item.serieId, item.size);
      else if (item.type === 'serie') {
        const serie = getSeries().find(s => s.id === item.serieId);
        if (serie) serie.items.filter(i => !i.sold).forEach(i => sellSerieItem(serie.id, i.size));
      }
    });
    setReceipt({ items: [...cart], subtotal, total, date: new Date().toLocaleString(), id: `REC-${Date.now().toString(36).toUpperCase()}`, customer: selectedCustomer });
    setCart([]); setScanLog([]); setLastScanned(null); setSelectedCustomer(null);
  };

  const handlePrint = () => {
    const printWindow = window.open('', '_blank', 'width=350,height=600');
    if (!printWindow || !receipt) return;
    const customerLine = receipt.customer ? `<div class="row"><span>Customer: ${receipt.customer.name}</span></div>` : '';
    printWindow.document.write(`<html><head><title>Receipt</title><style>body{font-family:'Courier New',monospace;width:280px;margin:0 auto;padding:20px 0;color:#1a1a1a}.center{text-align:center}.line{border-top:1px dashed #999;margin:10px 0}.row{display:flex;justify-content:space-between;margin:4px 0;font-size:13px}.bold{font-weight:bold}h2{margin:0 0 4px;font-size:18px}p{margin:2px 0;font-size:12px}</style></head><body><div class="center"><h2>CLOTHING STORE</h2><p>123 Fashion Ave, Suite 100</p><p>Tel: (555) 000-1234</p></div><div class="line"></div><div class="row"><span>Receipt: ${receipt.id}</span></div><div class="row"><span>${receipt.date}</span></div>${customerLine}<div class="line"></div>${receipt.items.map(item=>`<div class="row"><span>${item.name} x${item.quantity}</span><span>$${(item.price*item.quantity).toFixed(2)}</span></div>`).join('')}<div class="line"></div><div class="row bold" style="font-size:15px"><span>TOTAL</span><span>$${receipt.total.toFixed(2)}</span></div><div class="line"></div><div class="center"><p style="margin-top:12px">Thank you for shopping!</p><p>Have a great day</p></div></body></html>`);
    ...
    <div className="flex-1 flex h-full">
        ...
        <form onSubmit={handleScan} className="mb-6">
          <div className="relative">
            ...
            <input ref={inputRef} type="text" value={barcodeInput}
              ...
              className="w-full pl-12 pr-4 py-4 text-lg bg-primary/5 border-2 border-primary/30 rounded-xl focus:border-primary focus:outline-none transition-colors" autoFocus />
          </div>
          {scanError && <div className="flex items-center gap-2 mt-2 text-red-500 text-sm"><AlertCircle className="w-5 h-5" />{scanError}</div>}
        </form>
        ...
        {lastScanned && (
          <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-muted-foreground">{lastScanned.barcode} &middot; ${lastScanned.price.toFixed(2)}</div>
            </div>
          </div>
        )}
        ...
        {scanLog.length > 0 && (
          <div className="bg-card border border-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setScanLog([])} className="text-xs text-muted-foreground hover:text-foreground">Clear</button>
            </div>
            <div className="space-y-2 max-h-48 overflow-auto">
              {scanLog.map(entry => (
                <div key={entry.id} className={`flex items-center justify-between py-1.5 px-2 rounded text-sm ${entry.success ? 'text-foreground' : 'text-red-500 bg-red-50'}`}>
                  <div className="flex items-center gap-2">
                    {entry.success ? <CheckCircle2 className="w-3.5 h-3.5 text-green-600" /> : <AlertCircle className="w-3.5 h-3.5 text-red-500" />}
                    <span className="font-mono">{entry.barcode}</span>
                    {entry.productName && <span className="text-muted-foreground">&middot; {entry.productName}</span>}
                  </div>
      ...
      <div className="flex-[45] bg-card border-l border-border flex flex-col">
        <div className="p-6 border-b border-border flex items-center justify-between">
          <h2>Sale Items</h2>
          {cart.length > 0 && <button onClick={() => setCart([])} className="text-sm text-muted-foreground hover:text-red-600 flex items-center gap-1"><Trash2 className="w-5 h-5" />Clear</button>}
        </div>
        ...
        <div className="px-6 pt-4">
          <button onClick={() => setShowCustomerPicker(true)}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-muted-foreground truncate">{selectedCustomer ? selectedCustomer.email : 'Tap to assign a customer (optional)'}</div>
            </div>
            {selectedCustomer && <button onClick={e => { e.stopPropagation(); setSelectedCustomer(null); }} className="text-muted-foreground hover:text-foreground"><X className="w-4 h-4" /></button>}
          </button>
        </div>

        <div className="flex-1 overflow-auto p-6">
          {cart.length === 0 ? (
            <div className="text-center text-muted-foreground py-16"><ScanBarcode className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>Scan a barcode to start</p></div>
          ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {editingPriceId === item.id ? (
                      <form onSubmit={(e) => { e.preventDefault(); confirmEditPrice(item.id); }} className="flex items-center gap-1 mb-1">
                        ...
                        <input
                          ...
                          autoFocus
                          className="w-20 px-1.5 py-0.5 text-xs border border-primary rounded-md bg-primary/5 focus:outline-none focus:ring-1 focus:ring-primary font-mono"
                        />
                      </form>
                    ) : (
                      <button onClick={() => startEditPrice(item)} className="flex items-center gap-1 text-xs text-muted-foreground font-mono mb-1 hover:text-primary transition-colors group">
                    ...
                    {item.type === 'product' && (
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => updateQuantity(item.id, 1)} className="w-6 h-6 rounded bg-secondary hover:bg-muted flex items-center justify-center"><Plus className="w-3 h-3" /></button>
                      </div>
                    )}
                  </div>
                  <div className="flex flex-col items-end justify-between">
                    <button onClick={() => removeFromCart(item.id)} className="text-muted-foreground hover:text-destructive"><X className="w-5 h-5" /></button>
        ...
        <div className="p-6 border-t border-border space-y-6">
          <div className="space-y-3">
            ...
            <div className="flex justify-between pt-3 border-t border-border"><span>Total</span><span className="text-lg">${total.toFixed(2)}</span></div>
          </div>
      ...
      {receipt && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="p-6">
              <div className="text-center mb-4"><h2 className="text-lg">CLOTHING STORE</h2><p className="text-sm text-muted-foreground">123 Fashion Ave, Suite 100</p><p className="text-sm text-muted-foreground">Tel: (555) 000-1234</p></div>
              ...
              <div className="border-t border-dashed border-gray-300 my-3" />
              <div className="space-y-2">{receipt.items.map(item => (<div key={item.id} className="flex justify-between text-sm"><span>{item.name} x{item.quantity}</span><span>${(item.price * item.quantity).toFixed(2)}</span></div>))}</div>
              <div className="border-t border-dashed border-gray-300 my-3" />
            ...
            <div className="flex gap-3 p-4 border-t border-border">
              <button onClick={() => setReceipt(null)} className="flex-1 py-2.5 border border-border rounded-lg hover:bg-muted transition-colors">Close</button>
              <button onClick={handlePrint} className="flex-1 py-3 bg-primary text-primary-foreground rounded-lg hover:bg-primary/90 transition-colors flex items-center justify-center gap-2"><Printer className="w-5 h-5" />Print Receipt</button>
            </div>
          </div>
        </div>
      )}
